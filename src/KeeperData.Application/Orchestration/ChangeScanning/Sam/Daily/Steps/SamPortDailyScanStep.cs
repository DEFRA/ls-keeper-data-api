...
[StepOrder(5)]
public class SamPortDailyScanStep(
    IDataBridgeClient dataBridgeClient,
    IMessagePublisher<IntakeEventsQueueClient> intakeMessagePublisher,
    DataBridgeScanConfiguration dataBridgeScanConfiguration,
    IDelayProvider delayProvider,
    IConfiguration configuration,
    ILogger<SamPortDailyScanStep> logger)
    : DailyScanStepBase<SamDailyScanContext, SamScanPortIdentifier>(dataBridgeClient, intakeMessagePublisher, dataBridgeScanConfiguration, delayProvider, configuration, logger)
{
    private const string SelectFields = "CPH";
    private const string OrderBy = "CPH asc";

    protected override bool IsEntityEnabled()
        => Configuration.GetValue<bool>("DataBridgeCollectionFlags:SamPortsEnabled");

    protected override EntityScanContext GetScanContext(SamDailyScanContext context)
        => context.Ports;

    protected override async Task<DataBridgeResponse<SamScanPortIdentifier>?> QueryDataAsync(
        SamDailyScanContext context,
        CancellationToken cancellationToken)
        => await DataBridgeClient.GetSamPortsAsync<SamScanPortIdentifier>(
            context.Ports.CurrentTop,
            context.Ports.CurrentSkip,
            SelectFields,
    ...
    protected override async Task PublishMessagesAsync(
        DataBridgeResponse<SamScanPortIdentifier> queryResponse,
        CancellationToken cancellationToken)
    {
