...
public static class SamPartyMapper
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        await UpdateRoles(goldSiteId, incoming, goldSiteGroupMarks, getSpeciesTypeById, party, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var partyRole = await CreatePartyRole(goldSiteId, incoming, goldSiteGroupMarks, getSpeciesTypeById, role, cancellationToken);
        ...
        foreach (var goldParty in goldParties)
        {
            ...
            foreach (var partyRoleSite in party.Roles.Where(x => x.Site?.Id == goldSite.Id).Select(s => s.Site))
            {
                ...
                if (goldSite.Identifiers != null && goldSite.Identifiers.Count > 0)
                {
                    ...
                    partyRoleSite!.SetIdentifiers(identifiers);
                }
            }
