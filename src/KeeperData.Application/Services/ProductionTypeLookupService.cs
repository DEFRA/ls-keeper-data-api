...
public class ProductionTypeLookupService : IProductionTypeLookupService
{
    ...
    /// <returns></returns>
    public async Task<ProductionTypeDocument?> GetByIdAsync(string? id, CancellationToken cancellationToken)
    {
        if (string.IsNullOrWhiteSpace(id)) return null;

        return await Task.FromResult(new ProductionTypeDocument
        {
            IdentifierId = id
        });
    }
