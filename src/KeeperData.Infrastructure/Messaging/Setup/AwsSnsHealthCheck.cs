...
public class AwsSnsHealthCheck(IAmazonSimpleNotificationService snsClient, IBatchCompletionNotificationConfiguration config) : IHealthCheck
{
    private readonly IAmazonSimpleNotificationService _snsClient = snsClient ?? throw new ArgumentNullException(nameof(snsClient));
    private readonly IBatchCompletionNotificationConfiguration _config = config ?? throw new ArgumentNullException(nameof(config));

    public async Task<HealthCheckResult> CheckHealthAsync(
        HealthCheckContext context,
        CancellationToken cancellationToken = default)
    {
        var defaultTopicName = _config.BatchCompletionEventsTopic.TopicName;
        var defaultTopicArn = _config.BatchCompletionEventsTopic.TopicArn ?? string.Empty;
