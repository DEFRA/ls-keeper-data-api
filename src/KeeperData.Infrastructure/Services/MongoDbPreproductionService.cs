...
namespace KeeperData.Infrastructure.Services
{
    public class MongoDbPreproductionService : IMongoDbPreproductionService
    {
        ...
        public async Task<string> DropCollection(string collectionName)
        {
            ...
            var targetType = indexableTypes.FirstOrDefault(t => t.GetCustomAttribute<CollectionNameAttribute>()?.Name == collectionName);
            ...
            if (!_config.PermittedTables.Contains(collectionName))
                throw new ArgumentException($"Drop action is not permitted for collection '{collectionName}' ");

            await _db.DropCollectionAsync(collectionName, CancellationToken.None);
            await _initialiser.Initialise(targetType);

            return $"Dropped {collectionName}";
        }
    }
}