...
public class SamHerdDailyScanStepTests
{
    ...
    private readonly Mock<ILogger<SamHerdDailyScanStep>> _loggerMock = new();
    ...
    private readonly SamHerdDailyScanStep _scanStep;
    private readonly SamDailyScanContext _context;

    public SamHerdDailyScanStepTests()
    {
        _configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { { "DataBridgeCollectionFlags:SamHerdsEnabled", "true" } })
            .Build();

        _scanStep = new SamHerdDailyScanStep(
    ...
    public async Task ExecuteCoreAsync_ShouldQueryWithCorrectDateTimeFilter()
    {
        ...
        _dataBridgeClientMock.Verify(c => c.GetSamHerdsAsync<SamScanHerdIdentifier>(
            It.IsAny<int>(),
            It.IsAny<int>(),
            It.IsAny<string>(),
    ...
    [Fact]
    public async Task ExecuteCoreAsync_ShouldPublishSamUpdateHerdMessage()
    {
    ...
    public async Task ExecuteCoreAsync_ShouldBubbleException_WhenApiThrowsRetryableException()
    {
        ...
        var context = new SamDailyScanContext { CurrentDateTime = DateTime.UtcNow, UpdatedSinceDateTime = DateTime.UtcNow.AddHours(-24), Herds = new() };

        _dataBridgeClientMock
            .Setup(x => x.GetSamHerdsAsync<SamScanHerdIdentifier>(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
