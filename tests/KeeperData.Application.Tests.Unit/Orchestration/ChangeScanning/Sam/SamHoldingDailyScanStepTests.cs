...
public class SamHoldingDailyScanStepTests
{
    ...
    {
        var responseMock = MockSamData.GetSamHoldingsDataBridgeResponse(0, 0, 0);
        _dataBridgeClientMock
            .Setup(c => c.GetSamHoldingsAsync(5, 0, It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(responseMock);

        await _scanStep.ExecuteAsync(_context, CancellationToken.None);

        _dataBridgeClientMock.Verify(c => c.GetSamHoldingsAsync(
            It.IsAny<int>(),
            It.IsAny<int>(),
            It.IsAny<string>(),
    ...
    {
        var responseMock = MockSamData.GetSamHoldingsDataBridgeResponse(1, 1, 1);
        _dataBridgeClientMock
            .Setup(c => c.GetSamHoldingsAsync(5, 0, It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(responseMock);
