...
public class SamPortDailyScanStepTests
{
    ...
    private readonly Mock<ILogger<SamPortDailyScanStep>> _loggerMock = new();
    ...
    private readonly SamPortDailyScanStep _scanStep;
    private readonly SamDailyScanContext _context;

    public SamPortDailyScanStepTests()
    {
        _configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { { "DataBridgeCollectionFlags:SamPortsEnabled", "true" } })
            .Build();

        _scanStep = new SamPortDailyScanStep(
    ...
    [Fact]
    public async Task ExecuteCoreAsync_ShouldExitWhenSamPortsDisabled()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?> { { "DataBridgeCollectionFlags:SamPortsEnabled", "false" } })
            .Build();

        var scanStep = new SamPortDailyScanStep(
            _dataBridgeClientMock.Object,
            _messagePublisherMock.Object,
            _config,
            _delayProviderMock.Object,
        ...
        _dataBridgeClientMock.Verify(c => c.GetSamPortsAsync<SamScanPortIdentifier>(
            It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTime?>(),
            It.IsAny<string>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task ExecuteCoreAsync_ShouldMarkScanCompleted_WhenNoPortsReturned()
    {
        _dataBridgeClientMock
            .Setup(c => c.GetSamPortsAsync<SamScanPortIdentifier>(
                It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTime?>(),
                It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(new DataBridgeResponse<SamScanPortIdentifier> { CollectionName = "collection", Data = [] });

        await _scanStep.ExecuteAsync(_context, CancellationToken.None);

        Assert.True(_context.Ports.ScanCompleted);
    }

    [Fact]
    public async Task ExecuteCoreAsync_ShouldQueryWithCorrectDateTimeFilter()
    {
        var responseMock = MockSamData.GetSamPortsScanIdentifierDataBridgeResponse(0, 0, 0);
        _dataBridgeClientMock
            .Setup(c => c.GetSamPortsAsync<SamScanPortIdentifier>(5, 0, It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
    ...
    {
        var responseMock = MockSamData.GetSamPortsScanIdentifierDataBridgeResponse(1, 1, 1);
        _dataBridgeClientMock
            .Setup(c => c.GetSamPortsAsync<SamScanPortIdentifier>(5, 0, It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(responseMock);
    ...
    public async Task ExecuteCoreAsync_ShouldBubbleException_WhenApiThrowsRetryableException()
    {
        ...
        var context = new SamDailyScanContext { CurrentDateTime = DateTime.UtcNow, UpdatedSinceDateTime = DateTime.UtcNow.AddHours(-24), Ports = new() };

        _dataBridgeClientMock
            .Setup(x => x.GetSamPortsAsync<SamScanPortIdentifier>(It.IsAny<int>(), It.IsAny<int>(), It.IsAny<string>(), It.IsAny<DateTime?>(), It.IsAny<string>(), It.IsAny<CancellationToken>()))
