...
{
    private readonly Mock<CtsDailyScanOrchestrator> _orchestratorMock;
    ...
    private readonly Mock<ILogger<CtsDailyScanTask>> _loggerMock;
    private readonly Mock<IDelayProvider> _delayProviderMock;
    private readonly CtsDailyScanTask _sut;
    ...
    public CtsDailyScanTaskTests()
    {
        ...
        _loggerMock = new Mock<ILogger<CtsDailyScanTask>>();
    ...
    public async Task StartAsync_WhenApplicationStopping_ShouldLogWarning()
    {
        ...
        _orchestratorMock.Setup(x => x.ExecuteAsync(It.IsAny<CtsDailyScanContext>(), It.IsAny<CancellationToken>()))
            .Returns(async (CtsDailyScanContext ctx, CancellationToken token) =>
            {
        ...
        _loggerMock.Verify(x => x.Log(LogLevel.Warning, It.IsAny<EventId>(), It.Is<It.IsAnyType>((v, t) => v.ToString()!.Contains("Application is shutting down")), null, It.IsAny<Func<It.IsAnyType, Exception?, string>>()), Times.Once);
    }
    ...
    public async Task RunAsync_WhenLockAcquired_ShouldExecuteOrchestratorAndDisposeLock()
    {
        ...
        _orchestratorMock.Verify(x => x.ExecuteAsync(It.IsAny<CtsDailyScanContext>(), It.IsAny<CancellationToken>()), Times.Once);
    ...
    public async Task RunAsync_WhenLockNotAcquired_ShouldNotExecuteOrchestrator()
    {
        ...
        _orchestratorMock.Verify(x => x.ExecuteAsync(It.IsAny<CtsDailyScanContext>(), It.IsAny<CancellationToken>()), Times.Never);
    }
