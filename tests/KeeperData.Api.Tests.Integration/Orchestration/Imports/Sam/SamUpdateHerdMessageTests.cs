...
public class SamUpdateHerdMessageTests(IntegrationTestFixture fixture) : IClassFixture<IntegrationTestFixture>
{
    ...
    [Fact]
    public async Task GivenSamUpdateHerdMessagePublishedToQueue_WhenReceived_ShouldBeHandled()
    {
        var correlationId = Guid.NewGuid().ToString();
        var message = new SamUpdateHerdMessage { Identifier = CphGenerator.GenerateFormattedCphh("01") };
