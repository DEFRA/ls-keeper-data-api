...
public class PiiMaskingEnricherTests
{
    ...
    public void Enrich_MasksNestedUrlInStructure()
    {
        ...
        {
            new LogEventProperty("full", new ScalarValue("http://localhost/api/v2/cph-associations?email=secret@test.com")),
            new LogEventProperty("original", new ScalarValue("/api/v2/cph-associations?email=secret@test.com")),
        ...
        full!.Value.Should().Be("http://localhost/api/v2/cph-associations?email=***");

        var original = modifiedUrl!.Properties.Single(p => p.Name == "original").Value as ScalarValue;
        original!.Value.Should().Be("/api/v2/cph-associations?email=***");
    ...
    public void Enrich_RemovesTheRealEcsHttpContextPropertyWithoutMutatingTheRequest()
    {
        ...
        context.Request.Path = "/api/v2/cph-associations";
