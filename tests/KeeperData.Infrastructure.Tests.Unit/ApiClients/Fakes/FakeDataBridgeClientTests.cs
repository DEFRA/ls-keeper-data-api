...
public class FakeDataBridgeClientTests
{
    ...
    public async Task GetSamHerdsByPartyIdAsync_ShouldReturnCorrectDataBridgeResponse()
    {
        ...
        // Assert
        result.Should().NotBeNull();
        result!.Data.Should().HaveCount(2); // Based on implementation
    ...
    #region Data Generation and Randomization Tests
    ...
    public async Task GetSamHoldingsAsync_MultipleCalls_ShouldGenerateDifferentRandomData()
    {
        ...
        // CPH values should be different due to randomization
    ...
    #region DataBridgeResponse Structure Tests
    ...
    public async Task AllMethods_ShouldAcceptCancellationToken()
    {
        ...
        // If we get here, all methods accepted the cancellation token without throwing
    ...
    [Fact]
    public async Task GetCtsAgentByPartyIdAsync_WhenNoDataGenerated_ShouldReturnNull()
    {
        // This tests the edge case where GetCtsAgentOrKeeper returns empty list
        // We can't easily mock the internal random generation, but we test the null path exists

        // Act
        var result = await _sut.GetCtsAgentByPartyIdAsync("test-party-id", CancellationToken.None);

        // Assert
        // The method should either return a valid agent or null (based on internal logic)
        // We verify it doesn't throw and returns a consistent type
        if (result != null)
        {
